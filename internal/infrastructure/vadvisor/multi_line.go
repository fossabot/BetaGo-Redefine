...
type MultiSeriesLineGraph[X xconstraints.ValidType, Y xconstraints.Numeric] struct {
...
type DataStruct[X, Y xconstraints.ValidType] struct {
	Values []*DataValue[X, Y] `json:"values"`
}

type DataValue[X, Y xconstraints.ValidType] struct {
	SeriesField string `json:"seriesField"`
...
func NewMultiSeriesLineGraph[X xconstraints.ValidType, Y xconstraints.Numeric](ctx context.Context) *MultiSeriesLineGraph[X, Y] {
...
type (
	XYSUnit[X, Y xconstraints.ValidType] struct {
		X X      // X轴
		Y Y      // Y轴
		S string // 序列数据、分组
	}
